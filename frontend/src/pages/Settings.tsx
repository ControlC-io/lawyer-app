...
import { User, Bell, Shield, Save, Eye, EyeOff, Copy, RefreshCw, Building, Upload, Palette } from "lucide-react";
...
export default function Settings() {
  const { profile, refreshUserData, user, hasPermission } = useAuth();
  const { t } = useLanguage();
  const companyId = useCompanyId();
  const canManageOrg = hasPermission("org_settings.manage");
  ...
  useEffect(() => {
    if (companyId && canManageOrg) {
      void fetchCompanyData();
    }
  }, [companyId, canManageOrg]);
  ...
  return (
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">{t("sidebar.settings") || "Settings"}</h1>
      ...
      <Tabs defaultValue="account" className="space-y-6">
        <TabsList className="w-full justify-start">
          <TabsTrigger value="account">{t("sidebar.userSettings") || "User Settings"}</TabsTrigger>
          {canManageOrg && (
            <TabsTrigger value="general">{t("organizationSettings.tabs.general") || "General"}</TabsTrigger>
          )}
          {canManageOrg && (
            <TabsTrigger value="branding">{t("organizationSettings.tabs.branding") || "Branding"}</TabsTrigger>
          )}
        </TabsList>
