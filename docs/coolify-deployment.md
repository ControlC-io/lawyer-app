...
| `.mjs` files load with `application/octet-stream` — browser rejects them as module scripts | nginx's default `mime.types` only maps `application/javascript` to `.js`, not `.mjs` | In the frontend nginx `server` block, add a location-specific types override: `location ~* \.mjs$ { types { application/javascript mjs; } }` — do NOT use `include` inside a `types` block, it is invalid nginx syntax and crashes the container |
